...
\end{problem}
Equivalently, the goal is to find the smallest radius $r^*$ such that one can choose $k$ points in $P$ and place $k$ balls of radius $r^*$ around them to satisfy the coverage requirement for every color class. For a metric $(P,d)$ and for each $p \in P$ and $r \in \mathbb{R}_{\geq 0}$, we define the set $\mathcal{B}_P(p,r) \coloneqq \set{j \in P : d(j,p) \leq r}$ as the \emph{ball} of radius $r$ centered at $p$. If the set $P$ is clear from the context, it may be omitted from the subscript. For convenience, we define $\mathcal{B}(p) \coloneqq \mathcal{B}(p,1)$ for each $p \in P$.

Lately, we will focus our attention on the two color case. Before we will see that, assuming $\mathit{P} \neq \NP$, it is impossible to find an approximation algorithm for the problem if the number of colors is arbitrary. We are going to reduce the set cover problem into a real line instance of the colorful $k$-center problem and show that, if an approximation algorithm for this problem exists, we can answer the set cover instance in polynomial time.

\begin{theorem}
    Assuming $\mathit{P} \neq \NP$, the colorful $k$-center problem with an unbounded number of colors cannot be approximated to within any constant factor in polynomial time.
\end{theorem}

\begin{proof}
    Let $U$ be a finite set, $\mathcal{S} \subseteq 2^U$ be a family of subsets of $U$ and let $k \in \mathbb{Z}_{\ge 0}$. The Set Cover problem asks to decide wheter there exists a subset $\mathcal{S}' \subseteq \mathcal{S}$ such that $|\mathcal{S}'| \le k$ and $\bigcup_{S \in \mathcal{S}'} S = U$. \red{eu deveria colocar essa definição fora do proof environment?} Set $c \coloneqq |U|$ and $s \coloneqq |\mathcal{S}|$. Let $U = \set{u_1,\ldots, u_c}$ and $\mathcal{S} = \set{S_1,\ldots,S_s}$. Each element of $U$ will be represented as a color in the colorful $k$-center instance, with a coverage requirement of 1.

    We build the instance as follows. For each $i \in [s]$ and each $u \in S_i$, create a point $(i,u)$, and let $P \coloneqq \set{(i,u) : i \in [s],\, u \in S_i}$ be the point set of the instance, which has $c$ colors and $\sum_{i=1}^s |S_i|$ points. The point $(i,u)$ is assigned the color $u$, so that the color classes of $P$ are $P_u \coloneqq \set{(i,u) \in P : i \in [s]}$ for $u \in U$, each with coverage requirement 1. The distance is defined by $d\big((i,u),(i',u')\big) \coloneqq |i - i'|$, i.e., the point $(i,u)$ behaves as if it were placed at position $i$ of the real line. Look at the example in Figure~\ref{fig:reduction-set-cover-colorful}.
    \input{conteudo/tikz/hardnesscolkcenter.tex}
    We are going to prove that the answer of the Set Cover instance is ``yes'' if and only if the created instance of the colorful $k$-center problem has a solution of value 0.

    First, suppose that there is $ \mathcal{S}' \subseteq \mathcal{S}$ such that $|\mathcal{S}'| \leq k$ and $\bigcup_{S \in \mathcal{S}'} S = U$, i.e., the answer for the instance is ``yes''. Set $\mathcal{I} \coloneqq \set{i \in [|\mathcal{S}|] : S_i \in \mathcal{S}'}$ the indices of the subsets that compose the solution for the set cover. For each $i \in \mathcal{I}$, fix an arbitrary $u_i \in S_i$ and let $S \coloneqq \set{(i,u_i) : i \in \mathcal{I}}$, so that $|S| = |\mathcal{I}| \leq k$. Let $C \coloneqq \bigcup_{i \in \mathcal{I}} \set{(i,u) : u \in S_i}$. For each color $u \in U$, since $\bigcup_{i \in \mathcal{I}} S_i = U$, there is some $i \in \mathcal{I}$ with $u \in S_i$, hence $(i,u) \in C \cap P_u$, so the coverage requirement of every color is satisfied by $C$. Moreover, for every $(i,u) \in C$, one has $i \in \mathcal{I}$ and thus $d\big((i,u),(i,u_i)\big) = 0$ with $(i,u_i) \in S$. Therefore $\max_{j \in C} d(j,S) = 0$, and $(S,C)$ is a solution of value 0 for the colorful $k$-center instance.

    Now, suppose that the colorful $k$-center instance admits a solution $(S,C)$ of value 0, that is, $|S| \leq k$, the coverage requirement of every color is met by $C$, and $\max_{j \in C} d(j,S) = 0$. Let $\mathcal{I} \coloneqq \set{i \in [s] : (i,u) \in S \text{ for some } u \in S_i}$ be the set of positions occupied by the centers in $S$, so that $|\mathcal{I}| \leq |S| \leq k$. Fix a color $u \in U$. Since its coverage requirement is met, there is a point $(i,u) \in C \cap P_u$ for some $i \in [s]$ with $u \in S_i$. Since $\max_{j \in C} d(j,S) = 0$, there is a center $(i',u') \in S$ with $d\big((i,u),(i',u')\big) = |i - i'| = 0$, that is, $i' = i$, so $i \in \mathcal{I}$. Hence $u \in S_i$ for some $i \in \mathcal{I}$, i.e., $u \in \bigcup_{i \in \mathcal{I}} S_i$. As this holds for every $u \in U$, one has $U = \bigcup_{i \in \mathcal{I}} S_i$, so $\mathcal{S}' \coloneqq \set{S_i : i \in \mathcal{I}}$ is a family of at most $k$ sets covering $U$, and the answer of the Set Cover instance is ``yes''.

    To conclude, note that  any solution that is not of value 0 has value at least 1. Suppose, for the sake of contradiction, that there is a polynomial-time $\alpha$-approximation algorithm $\mathcal{A}$ for the colorful $k$-center problem with an unbounded number of colors, for some constant $\alpha \geq 1$. Given a Set Cover instance, build the colorful $k$-center instance as above and run $\mathcal{A}$ on it. If the answer to the Set Cover instance is ``yes'', the optimal value of the constructed instance is 0, so $\mathcal{A}$ must return a solution of value at most $\alpha \cdot 0 = 0$, i.e., of value exactly 0. If the answer is ``no'', the optimal value is at least 1, and the algorithm returns a feasible solution, necessarily of value at least 1. Thus, checking whether $\mathcal{A}$ returns a solution of value 0 decides the Set Cover instance in polynomial time, contradicting $\mathit{P} \neq \NP$, since Set Cover is $\NP$-hard.
\end{proof}
...
\red{Isso tem que ir para um capítulo de preliminares.} A pseudo $\alpha$-approximation for the colorful $k$-center problem is a polynomial-time algorithm that, given an instance $I = (R,B,\rho,\beta,d,k)$, finds sets $S$ and $C$ of points such that $|S| \leq k + O(1)$, $|C \cap R| \geq \rho$, $|C\cap B| \geq \beta$, and $\max_{j\in C}d(j,S) \leq \alpha \,\opt{I}$, where $\opt{I}$ is the optimal value for the instance. That is, the algorithm finds a solution that satisfies the coverage constraints and has a bounded value compared to the optimal one, but relaxes the constraint on the number of open centers. First, a pseudo 2-approximation algorithm for the colorful \(k\)-center problem, due to Bandyapadhyay, Inamdar, Pai, and Varadarajan~\cite{BIPV2019} is presented, which will be used as a subroutine in the JSS approximation.

\subsection{Pseudo approximation for the colorful $k$-center}

The pseudo approximation presented here is due to Bandyapadhyay \emph{et al.}~\cite{BIPV2019}, but has a simple modification due to Jia \emph{et al.}~\cite{JSS2020}, which will be presented here.
...
\begin{aligned}
\sum_{i\in \mathcal{B}(j)} \; x_i &\ge z_j,
  &&\forall\,j\in P,\\
...
In the first phase, \ref{LP1} is solved to generate a set of candidate clusters; in the second phase, at most \(k+1\) clusters are selected from these candidates. 
For the algorithm of the first phase, the following definition is used.
\begin{definition}
    For each $j \in P$, the set $\mathcal{F}(j) \coloneqq \bigcup_{i \in \mathcal{B}(j)}\mathcal{B}(i)$ is the \emph{flower set} of $j$.
\end{definition}
\begin{figure}[h]
    \centering
    \input{tikz/flower.tikz}
    \caption{Points in $P$ within the purple region form the flower set of point $j$.}
\end{figure}

\subsubsection{Phase one}

The first phase is given by  Algorithm~\ref{Alg1}, which builds a set of candidate clusters. 
First, an optimal solution~$(x,z)$ of~\ref{LP1} is taken. 
...
\begin{algorithm}
    ...
    \label{Alg1}
    \begin{algorithmic}[1]
        \State Let $(x,z)$ be a solution of~\ref{LP1} for $R, B, k, \rho, \beta$ using $d$. 
        ...
        \State $y_j \gets \min\set{1,\sum_{i\in\mathcal{B}(j)} x_i}$
...
By definition of $C_j$, one has $i \in \mathcal{F}(j)$. Thus, there is \(h \in \mathcal{B}(j)\) such that \(i \in \mathcal{B}(h)\). Therefore,
\[
d(i,j)
\;\le\; d(i,h) + d(h,j)
\;\le\; 1 + 1 \;=\; 2,
\]
where the first inequality holds by the triangle inequality, and the last by the definition of $\mathcal{B}(j)$ and $\mathcal{B}(h)$.
\end{proof}
...
\]
Therefore, if $y$ is a feasible solution of~\ref{LP2} with objective value at least $\rho$, then the candidate clusters satisfy the coverage requirements.

\begin{theorem}
    \label{feas:LP2}
The vector $y$ produced by {\rm Algorithm~\ref{Alg1}} is a feasible solution of~\ref{LP2} with objective value at least $\rho$.    
\end{theorem}
...
\begin{proof}
    ...
    \begin{equation}
        \label{z-bound}
    \tilde{z}_j = \sum_{h \in \mathcal{B}(j)} x_h \geq z_j \geq z_i, 
    \end{equation}
    where the first inequality holds because of the first constraint of~\ref{LP1} and the second inequality holds because $i$ was in $P'$ when $j$ was chosen to join $S$ and by the choice of~$j$ in line 4 of~{Algorithm~\ref{Alg1}}.

    Now, let us argue that $y$ is feasible for~\ref{LP2} with objective value at least $\rho$. For each $j \in S$, set $R_j \coloneqq C_j \cap R$. Note that 
    \begin{align*}
    ...
    \end{align*}
    where the first equality holds by the definition of $\tilde{z}$, the second equality holds by the definition of $r_j$, the first inequality holds by~\eqref{z-bound}, the third equality holds because, by construction, the $C_j$'s are disjoint and contain all points with positive $z$-values, and the last inequality holds by the third constraint of~\ref{LP1}. Similarly, \(\sum_{j \in S } b_jy_j \geq \beta\). Now, we must prove that \(\sum_{j \in S} y_j \leq k\). Note that
    \begin{align*}
    \sum_{j \in S} y_j \leq \sum_{j \in S} \sum_{i \in \mathcal{B}(j)}x_i \leq \sum_{i \in P} x_i \leq k,
    \end{align*}
    where the first inequality holds by the definition of $y$, the second holds from the claim that the unit balls centered at points in $S$ are disjoint, and the last one holds because $x$ satisfies the second constraint of~\ref{LP1}.
\end{proof}

The pseudo approximation is completed by using~\ref{LP2} to choose, in phase two, some clusters from the candidates.

\subsubsection{Phase two}

The second phase begins by utilizing the candidate clusters from phase one to define the constraints of~\ref{LP2}. 
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \EndFor
        \State Let $y^*$ be an extreme-point optimal solution of~\ref{LP2} for $S$ and vectors $r$ and $b$.
...
The blue requirement follows from the first constraint of~\ref{LP2}, and the red requirement holds because Theorem~\ref{feas:LP2} shows that there is a solution of~\ref{LP2} with objective value at least $\rho$. It remains to bound the size of $S^*$.
...
%
\begin{proof}
    Since~\ref{LP2} has exactly two non-trivial constraints, every extreme-point solution has at most two strictly fractional variables. This is a classic result from linear algebra which is implied by Lemma 2.1.4 in~\cite{LauRaviSingh2011}. If it has at most one strictly fractional variable, then the statement trivially holds. Thus, suppose that there are $i,i' \in S^*$ which have strictly fractional $y^*$-values. Then, one has
...
\caption{Non-well-separated-instances($R, B, \rho, \beta, d, k$)}
    ...
    \For{each $j \in R \cup B$}
        ...
        \If{\ref{LP1} is feasible for $R \setminus R_j, B \setminus B_j, \rho - |R_j|, \beta - |B_j|, d, k - 2$}
...
The algorithm iterates over all points in $R \cup B$. Consider the iteration where $j = j^*$. At this point, $R_{j^*} := R \cap \mathcal{B}(j^*, 3)$ and $B_{j^*} := B \cap \mathcal{B}(j^*, 3)$ are defined. Because $\mathcal{B}(j^*, 3)$ covers at least two unit balls centered at points of $\OPT$, the remaining set $\OPT \setminus \mathcal{B}(j^*, 3)$ has size at most $k - 2$ and the unit balls centered at them cover at least $\rho - |R_{j^*}|$ red points and $\beta - |B_{j^*}|$ blue points from $R \setminus R_{j^*}$ and $B \setminus B_{j^*}$.
Therefore, the optimal value of the instance $(R \setminus R_{j^*}, B \setminus B_{j^*}, \rho - |R_{j^*}|, \beta - |B_{j^*}|, d, k - 2)$ is also bounded by 1, which implies that~\ref{LP1} on them is feasible. As guaranteed by Theorem~\ref{pseudo_guarantee}, the BIPV algorithm will successfully return a pseudo solution $(S, C)$.
...
But it will be shown that, for $P_s$, some constraints can be added in~\ref{LP1} so that, for each candidate cluster center $j$, it holds that $|\mathcal{F}(j) \cap R \cap P_s| \leq 3\tau$.
...
\end{lemma}
\begin{proof}
The algorithm $\mathcal{A}_s$ is now described. By the assumption, there exist $k_s$ unit balls centered at points in $\OPT \cap P_s$ that cover $\beta_s$ blue points and $\rho_s$ red points of $P_s$. Hence, \ref{LP1} has a feasible solution $(x,z)$ for the instance defined by the point set $P_s$, the number of clusters $k_s$, and the covering requirements $\beta_s$ and $\rho_s$.
