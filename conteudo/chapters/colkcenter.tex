...
\begin{proof}
    Let $U$ be a finite set, $\mathcal{S} \subseteq 2^U$ be a family of subsets of $U$ and let $k \in \mathbb{Z}_{\ge 0}$. The Set Cover problem asks to decide wheter there exists a subset $\mathcal{S}' \subseteq \mathcal{S}$ such that $|\mathcal{S}'| \le k$ and $\bigcup_{S \in \mathcal{S}'} S = U$. \red{eu deveria colocar essa definição fora do proof environment?} Set $c \coloneqq |U|$ and $s \coloneqq |\mathcal{S}|$. Let $U = \set{u_1,\ldots, u_c}$ and $\mathcal{S} = \set{S_1,\ldots,S_s}$. Each point of $U$ will be represented as a color in the colorful $k$-center instance with a coverage requirement of 1. We will create a instance of the colorful $k$-center with $c$ colors and $\sum_{i = 1}^s |S_i|$ points. For each $i \in [s]$, we create a set of points placed in the position $i$ of the real line. For each element of $S_i$, we place one point with the color that represents such element in the position $i$. Note that, after doing that with each point of $S_i$, this is equivalent to have one point with multiple colors. Look at the example in Figure~\ref{fig:reduction-set-cover-colorful}.
