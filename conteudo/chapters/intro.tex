...
In these variants, known as the \emph{robust $k$-center} and \emph{robust $k$-median} problems, formulated by~\cite{CharikarKMN2001}, the input is the same as in the standard version, with the addition of an integer $t$ representing the minimum number of clients that must be served. 
...
Using the disparate impact doctrine, as articulated by~\cite{FSMSV2015} following the \href{https://en.wikipedia.org/wiki/Griggs_v._Duke_Power_Co.}{\emph{Griggs v.\ Duke Power Co.}\ US Supreme Court case}, the color classes can be viewed as protected attributes, such as race and gender. 
...
For the $k$-center problem, \cite{G1985} and, independently, \cite{HS1985} gave 2-approximation algorithms for the problem, while \cite{HN1979} proved that an approximation algorithm with a better ratio exists only if $P=\NP$. 
For the $k$-median problem, the best approximation algorithm known is a very recent $(2 + \eps)$-approximation by~\cite{CGLS2025}, while~\cite{JMS2002} showed that it is $\NP$-hard to approximate the problem with a ratio better than $1.763$.

For the version of the $k$-center that allows outliers, that is, the robust ${k\text{-center}}$,~\cite{CGK2020} designed a 2-approximation. 
...
For the $k$-median with outliers, the best known approximation algorithm is a $(6.994 + \eps)$-approximation designed by~\cite{GMZ2021}.
\cite{CHXXZ2024} developed a fixed-parameter tractable algorithm that produces a solution of cost at most $3 + \eps$ times the minimum cost. Its running time is polynomial on the size of $D$ and $F$ but is exponential on $k$.

For the colorful $k$-center, there is an algorithm that nearly matches the tight approximation guarantee of 2. \cite{JSS2020} designed a 3-approximation algorithm for this problem, which takes polynomial time as long as the numbers of colors is constant.
For the colorful $k$-median problem, there is a fixed-parameter tractable algorithm in $k$ and $m$, where $m$ is the total number of outliers among all colors, which is a $(1 + \frac{2}{e} + \eps)$-approximation by~\cite{AISX2024}. The approach used reduces a colorful $k$-median instance into multiple instances of the standard version of the $k$-median problem (without outliers or colors restrictions).
