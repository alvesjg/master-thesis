...
\usepackage[thesis, bibstyle=numeric]{imelooks}
% \usepackage[thesis]{imelooks}

% Correção para um bug conhecido de titlesec + hyperref: sem isto, o
% destino/marcador de PDF de um título de seção pode ser "roubado" pela
% última equação numerada anterior (gera avisos de "destination with the
% same identifier"), fazendo o link do sumário/marcadores apontar quase,
% mas não exatamente, para o título certo.
% https://tex.stackexchange.com/questions/97873
\makeatletter
\pretocmd{\ttlh@hang}{\phantomsection}{}{}
\pretocmd{\ttlh@display}{\phantomsection}{}{}
\pretocmd{\ttlh@runin}{\phantomsection}{}{}
\makeatother
...
% Ambientes de teorema/lema/definição/etc usados nos capítulos. amsthm e
% thmtools já são carregados por imegoodies; aqui apenas declaramos os
% ambientes que faltam (veja conteudo/05-exemplo-exemplos.tex para mais
% exemplos de como criar/personalizar esses ambientes).
...
% Macros matemáticas usadas nos capítulos (amssymb/amsmath/mathtools já são
% carregados por imegoodies)
\newcommand{\red}[1]{\textcolor{red}{#1}}
\newcommand{\set}[1]{\left\{#1\right\}} % notação de conjunto: $\set{a,b,c}$
\newcommand{\real}{\mathbb{R}} % números reais: $\real_{\geq 0}$
\DeclareMathOperator{\OPT}{OPT} % valor ótimo: $\alpha\,\OPT$; ou conjunto de centros de uma solução ótima (dependendo do capítulo): $c \in \OPT$
\newcommand{\opt}[1]{\mathrm{opt}(#1)} % valor ótimo de uma instância $I$: $\opt{I}$

% Pseudocódigo (ambientes algorithm/algorithmic com \State, \While etc);
% veja os comentários sobre pseudocódigo em imelooks.sty para outras opções
\usepackage{algorithm}
\usepackage{algpseudocodex}

% Permite que \input{tikz/arquivo.tikz} nos capítulos (que ficam em
% conteudo/chapters/) encontre os arquivos em conteudo/tikz/, já que os
% caminhos de \input são relativos ao diretório de tese.tex
\makeatletter
\def\input@path{{conteudo/}}
\makeatother
...
\title{Clustering with fairness and outliers}
\translatedtitle{Clustering com fairness e outliers}
...
\banca{
  \profa{} \dra{} Cristina Gomes Fernandes -- IME-USP [sem ponto final],
  % Em inglês, não há o "ª"
  %Prof. Dr. Fulana de Tal (advisor) -- IME-USP [sem ponto final],
  \profa{} \dra{} Carla Negri Lintzmayer -- UFABC [sem ponto final],
  Prof. Dr. Lehilton Lelis Chaves Pedrosa -- Unicamp [sem ponto final],
}
...
\apoio{During the development of this work, the author received\\
     financial support from FAPESP -- grant \#2025/04638-7}
...
\input{conteudo/chapters/intro}
\pagestyle{mainmatter}
\input{conteudo/chapters/colkcenter.tex}
\input{conteudo/chapters/kmedout.tex}
\input{conteudo/chapters/colkmed.tex}
